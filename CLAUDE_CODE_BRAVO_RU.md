...
Тот же ключ подходит не только Claude Code. Его можно выдать pet-проекту,
внутреннему сервису или агентному framework, который принимает OpenAI- либо
Anthropic-совместимые base URL и key. Это позволяет команде отлаживать проект
на разрешённых подписках, не переписывая его сразу под прямой платный API.
Agent SDK при этом не обязателен и не запрещён: Bravo заменяет транспорт и
маршрутную policy, а не прикладную агентную логику.
