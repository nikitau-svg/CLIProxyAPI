...
| --- | --- | --- |
| **Bravo 0.9.0-preview.11** | current preview | Безопасная 0.8.11 плюс управляемый адаптивный edge gate (`observe` по умолчанию, явный `enforce` для production), строгий порядок «своя primary → общий пул» с защитой резервов владельца, общий ограниченный HTTP/2 transport pool и error-only логи в режиме `metadata` без тел запросов и ответов. Адаптивный режим не ставит запросы в очередь и не добавляет обращений к провайдеру. |
...
Публичный канал Bravo — **`bravo/main`**; сейчас он ведёт на preview.11.
...
### Что именно добавляет preview.11
...
2. После smoke-проверки явно включите `enforce`. Он может пропустить только
   подтверждённо опасную локальную попытку и сразу продолжить уже настроенный
   соседний маршрут; неизвестная, устаревшая или переполненная runtime-оценка
   остаётся fail-open.
3. Проверьте `/v0/management/bravo/adaptive-audit`: отсутствие очередей и
   ...
   quota outcomes. При сомнении верните `observe` без отката образа.
