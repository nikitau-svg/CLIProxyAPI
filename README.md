...
| --- | --- | --- |
| **Bravo 0.9.0-preview.13** | beta candidate | Preview.12 плюс отдельный opt-in `assist`: только полностью токен-калиброванная secondary-попытка с актуальной подтверждённой квотой может быть атомарно перенесена в последовательный хвост того же запроса. Кандидат не удаляется, primary не затрагивается, streaming hedge отключён, а при конечном `max_attempts` прогноз полностью fail-open. Evidence-breaker остаётся активным. Семидневный почасовой audit-sidecar исправляет недостижимый readiness при потоке выше 4096 запросов за шесть часов и не хранит идентификаторы или содержимое запросов. `assist` предназначен для изолированной beta-canary; prod-default остаётся `breaker`, полный `enforce` не готов. |
| **Bravo 0.9.0-preview.12** | current production preview | Безопасный production-режим `breaker`: маршрут временно закрывается только после реальной проверенной quota/rate-limit ошибки, прогноз остаётся shadow-only, а локальное adaptive-решение никогда не выдаётся клиенту. После обычных соседей Bravo допускает не более одного глобального single-flight recovery probe на поколение breaker, если не исчерпан операторский `max_attempts`; остальные запросы не ждут и не спамят подписку. |
...
Публичный production-канал Bravo — **`bravo/main`**; сейчас он ведёт на
preview.12. Preview.13 до отдельного решения публикуется только как beta-
кандидат и не является источником обновления AWS.
...
### Что именно добавляет preview.13

Накопленный backtest показал, что жёсткий прогнозный admission пока опасен:
часть реально успешных попыток он хотел бы удержать, а часть настоящих quota-
отказов всё равно пропускает. Поэтому `preview.13` не включает полный
`enforce`. Новый `assist` использует прогноз только как мягкое предпочтение:
подтверждённо свежая и полностью токен-калиброванная secondary-попытка, чей
прогноз не помещается в доступный остаток, переносится в хвост уже разрешённого
плана. Сначала пробуются обычные соседи; если они не ответили, исходный точный
кандидат всё равно получает последовательный fail-open шанс. Его provider,
model, auth и project не перестраиваются из общего пула.

`assist` не применяется к primary, `/compact`, allocator bypass, breaker probe,
неполной калибровке, неизвестной или устаревшей квоте и любому конечному
`max_attempts`. Он не ждёт, не создаёт очередь и не запускает фоновые обращения.
Streaming-запрос отключает hedge только когда request-snapshot `assist` реально
может откладывать попытки (`max_attempts: 0`), поэтому отложенный tail не может
стартовать параллельно. При конечном `max_attempts` soft assist полностью inert
и сохраняет обычный streaming hedge. Evidence-based breaker из
preview.12 остаётся более сильным ограничением и повторно проверяется перед
физической отправкой tail. Одного глобального режима недостаточно: soft assist
включается только у проекта с явным `adaptive_assist: true`; остальные проекты
на том же сервере продолжают работать как `breaker`.

Старый detailed audit намеренно держал только последние 4096 запросов. При
нагрузке выше примерно 11 запросов в минуту эти детали не могли покрыть
обязательные шесть часов, поэтому readiness был математически недостижим.
Теперь отдельный ограниченный семидневный почасовой sidecar хранит только
агрегированные счётчики и временные границы. Он переживает перезапуск без
миграции, использует назначаемую writer-последовательность и checkpoint,
восстанавливает более новый JSONL-хвост без потерь и двойного учёта и явно
показывает усечение истории и причины, мешающие readiness. Отдельные assist-
счётчики связывают defer и точный tail внутри одного запроса, показывают
success/failure canary и принудительно дают `needs_review` при потере, дубле,
primary-defer или streaming hedge.
...
3. Проверьте `/v0/management/bravo/adaptive-audit`: отсутствие очередей и
   ...
   quota outcomes. Поля `oldest_retained_at`, `retained_history_span_seconds`,
   `history_truncated`, `high_rate_truncation` и `readiness_blockers` отделяют
   объём данных от их реальной пригодности.
4. `assist` включайте только при `max_attempts: 0`: установите глобальный режим
   `assist`, затем включите `adaptive_assist: true` ровно у одного отдельного
   некритичного beta-проекта через существующий Management API проектов. Все
   остальные проекты останутся breaker-only. Узкий kill switch — снять у
   проекта `adaptive_assist`; общий — вернуть режим `breaker`. Проверяйте
   отдельные `assist_requests`, success/failure, deferred/tail и invariant-
   счётчики. Полный прогнозный `enforce` остаётся экспериментальным и не готов
   к общему пулу.
5. Полный аварийный откат — прежний образ 0.8.11 с сохранёнными auth/config и
