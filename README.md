...
| --- | --- | --- |
| **Bravo 0.9.0-preview.4** | shadow preview | Безопасная 0.8.11 плюс адаптивный сбор и рейтинг расхода подписок. Исправлена математика первого неполного часа, а накопительный аудит снова виден отдельным блоком. Режим только наблюдает: он не блокирует запросы, не меняет порядок маршрутов и не создаёт дополнительных обращений к провайдерам. |
